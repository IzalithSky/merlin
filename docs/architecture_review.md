...
|---|---|---|---|---|
| 5 | God objects / mixed responsibilities in controller, bot pilot, and spawner | Architecture | High | Open — split plan active; phases 1–2 landed |
...
## 5. God objects / mixed responsibilities — High — OPEN (phases 1–2 done)
...
- `plane_character_controller.gd` — **2,019 lines** (was 1,864 at the second pass): flight model, input collection/smoothing, bot-facing control surface, net-input application, prediction history, reconciliation, remote interpolation, snapshot build, crash damage, sustain-turn/Vy limiter math, debug rendering, aero-table persistence.
...
**Phases 1–2 landed.** `plane_bot_pilot.gd` now delegates its pitch-rate stabilization calls to `PlaneCharacter.get_rate_stabilized_axis_input` / `get_rate_stabilized_input_for_desired_rate`, matching the existing roll delegation and deleting the duplicated control-law helpers. Networking state sync is now extracted into `scripts/plane_net_adapter.gd` (**291 lines**), and `plane_character_controller.gd` delegates prediction, reconciliation, interpolation, snapshot build, and input seq/ack bookkeeping through that adapter. `tests/run_headless_smokes.sh` and headless boots of `res://scenes/world_0.tscn` and `res://scenes/bot_duel.tscn` passed after the refactor.
