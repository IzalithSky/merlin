...
func _on_projectile_entered(node: Node) -> void:
	...
	for peer_id in multiplayer.get_peers():
		if _spawner.is_peer_world_ready(peer_id):
			cl_spawn_missile.rpc_id(peer_id, missile_id, missile.global_transform, velocity, target_peer_id)
...
func cl_spawn_missile(missile_id: int, transform_value: Transform3D, velocity: Vector3, target_peer_id: int) -> void:
	if multiplayer.is_server():
		return

	var missile := MISSILE_SCENE.instantiate() as Missile
	_projectiles.add_child(missile)
	missile.init_replica(transform_value, velocity, _resolve_remote_missile_target(target_peer_id))
...
func _resolve_remote_missile_target(target_peer_id: int) -> Node3D:
	if target_peer_id < 0:
		return null
	return _spawner.get_character(target_peer_id)
