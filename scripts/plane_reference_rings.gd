...
func _redraw_rings() -> void:
	...
	_append_ring(
		center - plane_up * outer_down_offset,
		outer_radius,
		plane_right,
		plane_forward,
...
func _append_ring(
	...
	var major_segments := maxi(ring_segments, 8)
	...
	for major_index in range(major_segments):
		var major_angle_from := (TAU * float(major_index)) / float(major_segments)
		var major_angle_to := (TAU * float(major_index + 1)) / float(major_segments)
		var radial_from := axis_a_normalized * cos(major_angle_from) + axis_b_normalized * sin(major_angle_from)
		var radial_to := axis_a_normalized * cos(major_angle_to) + axis_b_normalized * sin(major_angle_to)
		var center_from := center + radial_from * resolved_radius
		var center_to := center + radial_to * resolved_radius
		var tube_normal_from := radial_from.normalized()
		var tube_normal_to := radial_to.normalized()
		var tube_binormal_from := axis_a_normalized.cross(axis_b_normalized).normalized()
		var tube_binormal_to := tube_binormal_from
		...
		for minor_index in range(minor_segments):
			...
			var offset_from_a := (
				tube_normal_from * cos(minor_angle_from) + tube_binormal_from * sin(minor_angle_from)
			) * half_thickness
			var offset_from_b := (
				tube_normal_from * cos(minor_angle_to) + tube_binormal_from * sin(minor_angle_to)
			) * half_thickness
			var offset_to_a := (
				tube_normal_to * cos(minor_angle_from) + tube_binormal_to * sin(minor_angle_from)
			) * half_thickness
			var offset_to_b := (
				tube_normal_to * cos(minor_angle_to) + tube_binormal_to * sin(minor_angle_to)
			) * half_thickness
