...
func _detach_world_child(world: Node, child_name: String) -> void:
	...
	if child == null:
		push_warning("Bot chase scene could not find world child: %s" % child_name)
		return
...
func _killzone() -> KillzoneCone:
	return KillzoneCone.new(bot_killzone_min_distance, bot_killzone_max_distance, bot_killzone_base_radius)
...
func _update_killzone_marker() -> void:
	...
	KillzoneCone.append_frustum_lines(
		_killzone_mesh,
		cone.min_point(_dummy_target),
		cone.max_point(_dummy_target),
		cone.radius_at(cone.min_distance),
		cone.base_radius,
		Color(0.2, 1.0, 0.15, 0.9),
		KILLZONE_MARKER_SEGMENTS
	)
...
func _update_score(delta: float) -> void:
	...
	_distance_to_killzone = _killzone().distance_to(_bot.global_position, _dummy_target)
